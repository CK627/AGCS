...
start "Dashboard" python backend\app.py --port %PORT%
echo 服务已启动
...
REM 按窗口标题杀
tasklist /FI "WINDOWTITLE eq Dashboard" 2>nul | find "python.exe" >nul
if !errorlevel! equ 0 (
...
python -c "import flask, waitress, pymavlink, numpy" >nul 2>&1
if !errorlevel! equ 0 (
    echo   ✓ 核心依赖已安装
) else (
...
REM ============================================
:do_help
echo Dashboard 管理脚本 (Windows)
echo.
echo 用法:  start.bat [命令] [参数]
echo.
echo 命令:
echo   start [端口]   启动服务（默认端口 20000）
echo   stop           停止服务
