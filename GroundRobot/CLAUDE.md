...
- **当前卡点**：NO6/NO7 第一次夹取后的「拔起」方向/幅度。动的是 22 号「肩」，复位脉宽 705，**比 705 大才是上抬**（现场已验证：`785` 抬得太高）。当前代码里 `PULL_UP_22 = 400`，**低于 705，按这个规律是往下压的方向**，且接近夹取位 `22:395`；该值**尚未现场执行过**，别当已验证值用。试值走 `--pull-up N`，建议 705~785 之间，不用改代码。
