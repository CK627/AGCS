...
from agcs_lib.vision import correct_camera


class Searcher:
    """detect: callable，返回 detect_color 的 dict 或 None。"""

    def __init__(self, board, ik, ak, params, detect, ultrasonic=None, display=None, tof=None, depth=None):
        self.board = board
        self.ik = ik
        self.ak = ak
        self.params = params
        ...
        self.depth = depth  # 深度相机(DepthCamera，已 open+start_depth)，逼近判距用；None 则退回面积
        self.rotate = params['vision'].get('camera_rotate', 0)
        ...
        self.stop_depth_cm = float(sc.get('stop_depth_cm', 25.0))  # 深度判距停止距离(cm)
    ...
    def _depth_cm_at(self, cx, cy):
        """读深度相机在彩色像素 (cx, cy) 处的距离(cm)；无有效读数返回 None。

        彩色帧与深度帧同为 640×480，先按 camera_rotate 对齐方向，再取目标
        像素的深度值。深度 0 表示无效（太近/无数据），返回 None 交给面积兜底。
        """
        if self.depth is None:
            return None
        try:
            d = self.depth.read_depth(timeout_ms=200)
            if d is None:
                return None
            if self.rotate:
                d = correct_camera(d, self.rotate)
            h, w = d.shape
    ...
    def _approach(self, det):
        ...
        for step in range(self.max_approach):
            ...
            # 到位判定：优先深度判距（stop_depth_cm），深度无效再退回面积阈值兜底
            d_cm = self._depth_cm_at(cx, cy)

            self.log.debug('[search] %s', action_msg(
                '追踪 #%d' % (step + 1),
                action='云台水平=%d脉宽 云台俯仰=%d脉宽 中心x=%dpx 中心y=%dpx 半径=%dpx 21偏移=%+d 面积=%d 距离=%s'
                % (r['x_dis'], r.get('y_dis', self.base_pulse), cx, cy, radius, dx, area,
                   ('%.1fcm' % d_cm) if d_cm is not None else '无')))

            stop_by_depth = d_cm is not None and d_cm <= self.stop_depth_cm
            stop_by_area = d_cm is None and area >= self.approach_area_threshold
            if stop_by_depth:
                self.log.info('[search] %s', action_msg(
                    '到位(深度阈值)', reason='距离=%.1fcm <= %.1fcm' % (d_cm, self.stop_depth_cm),
                    action='停止逼近交给夹取'))
            elif stop_by_area:
                self.log.info('[search] %s', action_msg(
                    '到位(面积阈值,深度无效)', reason='面积=%d >= %d' % (area, self.approach_area_threshold),
                    action='停止逼近交给夹取'))

            if stop_by_depth or stop_by_area:
