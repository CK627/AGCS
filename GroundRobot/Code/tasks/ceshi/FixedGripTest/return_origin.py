#!/usr/bin/python3
# coding=utf8
"""按 record_route.py 录制的 fixed_route.json 反向回到原点。"""
...
def main():
    parser = argparse.ArgumentParser(description='反向执行录制路线回到原点')
    ...
    print('共 %d 个动作，开始反向回原点' % len(actions), flush=True)
    ik.stand(ik.initial_pos, t=500)
    time.sleep(0.5)

    for i, act in enumerate(reversed(actions), 1):
        name = act.get('action')
