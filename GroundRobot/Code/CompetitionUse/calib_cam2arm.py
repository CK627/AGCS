#!/usr/bin/python3
# coding=utf8
"""手眼标定：求彩色相机坐标系 -> 机械臂坐标系 的 R_cam2arm / t_cam2arm。

（从 git 历史 9ef5f09^ 取回，并更新：相机走 agcs_lib.camera + 用 camera_cal.yaml 的
彩色相机内参，不再用 MarkerDetector 默认的深度近似内参。）
...
def main():
    # 彩色相机内参：config/camera_cal.yaml 的 block_params/card_params K
    detector = MarkerDetector(marker_size_mm=100.0,
                              fx=424.606, fy=425.626,
                              cx=302.625, cy=276.322)
    cam = open_camera()
