#!/usr/bin/python3
# coding=utf8
"""Auto-capture.py —— 融合导航 + JSON 路线 + YOLO/深度夹取（独立脚本，只依赖 agcs_lib）。
...
**收尾的唯一判据是「模型认不出了」，不是「估距够近了」也不是「框顶快贴边了」**：
只要模型还能认出目标（哪怕框顶已经贴到画面上边）就继续靠近，等真的认不出了才收尾：
丢目标且锁定估距 ≤LOST_NEAR_CM → 24 交还给路线 JSON 的夹取角（24 和夹爪同轴，一路跟着
目标走会把夹爪带歪 → 夹不到），22/23 按锁定的估距盲走完剩下的路再夹（见 finish_blind）。
另外两个出口是**正常夹取**不是收尾：估距真到 STOP_DIST_CM 就夹（或深度 ≤STOP_DEPTH_CM）；
手臂伸到 JSON 夹取位 +REACH_EXTRA 的终点也夹（24 同样先还给 JSON）。
...
# 共享视觉/推流/模型封装（SourceCode/vision_common.py）
_SOURCE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'SourceCode')
if _SOURCE_DIR not in sys.path:
    sys.path.insert(0, _SOURCE_DIR)
from vision_common import open_vision, ModelDetector, video_loop, depth_cm_at
...
MODEL_CONF = 0.6                   # 置信度阈值。v8n.onnx 是 320×320 输入，远处 conf 只有
                                   # 0.80~0.84 贴着 0.8 一抖就丢，降到 0.6（19-21 现场）；
                                   # 代价是可能误检。识别稳了之后可再往上收。
...
#
# **提前收尾的两条都删了**（FINISH_CM=14，以及「框顶 ≤CAM24_TOP_MARGIN 就收尾」，
# 2026-09-21 现场连着删）——两次都是「明明还看得见却判成看不见」：
#   · 13.3cm 时模型还认得出（conf 0.93、框顶 59 在画面里），被 14cm 判成收尾，
#     22/23 只伸到 490/305 就停（终点 380/340），白丢 22 步行程 → 差一点夹不到；
...
def do_pick(board, model_det, depth, rotate, pick_count, pulses,
    ...
    if not observed:
        ...
        #     **只要模型还认得出就一直靠近**，不看「估距够近了没」也不看「框顶是不是贴边了」。
        #     等真的认不出了才收尾：24 交还给 JSON 夹取角，22/23 按锁定的估距盲走完剩下的
        #     路再夹；手臂伸到终点则是正常夹取（只把 24 还给 JSON）。
        ...
        for step in range(APPROACH_STEPS):
            ...
            if hit >= GRAB_HOLD:
                break
            # 这里**故意没有**提前收尾的判据（「估距够近」和「框顶快贴边」两条都于
            # 2026-09-21 删了）：模型还认得出就继续靠近。正常出口在上面
            # hit>=GRAB_HOLD（估距真到 STOP_DIST_CM）；真认不出了走上面丢目标那条；
            # 手臂伸不动了走循环末尾那条。框顶贴边不再算「看不见」——18-11 现场框顶 28
            # 时 conf 还有 0.90，那就是还看得见。
            # 21：水平跟踪，把目标拉回画面中心
